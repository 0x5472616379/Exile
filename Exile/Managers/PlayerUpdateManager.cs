...
public class PlayerUpdateManager
{
    ...
    private static void AppendAppearance(Player player, RSStream playerFlagUpdateBlock)
    {
        ...
        updateBlockBuffer.WriteByte((byte)player.Attributes.SkullIcon);
        updateBlockBuffer.WriteByte((byte)player.Attributes.HeadIcon);
        ...
        var t = player.Session.Username.ToLong();
        updateBlockBuffer.WriteQWord(t);
        updateBlockBuffer.WriteByte(126);
        updateBlockBuffer.WriteWord(100);

        var size = updateBlockBuffer.CurrentOffset;
        
        playerFlagUpdateBlock.WriteByte(size);
        
        byte[] reversedBuffer = new byte[updateBlockBuffer.CurrentOffset];
        Array.Copy(updateBlockBuffer.Buffer, 0, reversedBuffer, 0, updateBlockBuffer.CurrentOffset);
        Array.Reverse(reversedBuffer);

        playerFlagUpdateBlock.WriteBytes(reversedBuffer, reversedBuffer.Length, 0);
    }

    private static void UpdateCurrentPlayerMovement()
    {
        _player.Session.Writer.CreateFrameVarSizeWord(ServerOpCodes377.PLAYER_UPDATE);
    ...
    private static void WritePositionUpdate(Player player)
    {
        ...
        player.Session.Writer.WriteBits(1, player.Flags != PlayerUpdateFlags.None ? 1 : 0); // UpdateRequired aka does come with UpdateFlags
    }
